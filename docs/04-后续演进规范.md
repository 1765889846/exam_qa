...
> 2026年8月 · 与当前仓库进度对齐
> **当前基线**：P0–P2-B 已落地；P2-C 已完成工具底座，LLM 自主决策环待接入
...
| **P2-C** | 决策型 Agent | function-calling 工具底座已完成；LLM 决策环待接入 | P2-B、模型支持 tools |
| **P3** | 平台化 | 用户登录、课程 ACL、PostgreSQL + pgvector、异步入库 | P2、有明确需求再做 |

**当前进度**：P0 → P0.5 → P1-A → P1-B → P2-A → P2-B 已完成；P2-C 处于“工具底座完成、决策环待接入”状态；P3 按需启动。
...
### 4.4.8 决策 + 执行型 Agent（P2-C · 底座已完成）

> 让模型自己决定「下一步做什么」，而非固定状态图 + 规则分支。**当前工具层已落地，LLM 决策环与 API 开关尚未接通。**
...
1. P2-B 固定循环已稳定，`/ask` 与 `/agent/run` 仍是当前可用主路径。
2. `agent/tools.py` 已提供五个只读工具、OpenAI function schema、白名单分发与 `course_id` 强制注入。
3. 工具层单测已覆盖 schema 完整性、未知工具拒绝和分发。
4. `llm.py` 尚无 `chat_with_tools`，Agent state/graph 也尚无 `messages → tool_calls → tool result` 决策循环。
5. `AgentRunRequest` 尚无 `agentic` 开关，因此现有 `/agent/run` 不会调用 P2-C 工具注册表。
...
| `src/services/llm.py` | 待实现 | 新增 `chat_with_tools(messages, tools)`，透传 `tools` / `tool_choice` 并返回 `tool_calls` |
| `src/services/agent/state.py` | 待实现 | 新增 `messages` 及工具调用观测字段 |
| `src/services/agent/nodes.py` | 待实现 | 新增 `agent_node` 与 `tool_node` |
| `src/services/agent/graph.py` | 待实现 | 保留 P2-B 固定图，新增或分支到 `agent ↔ tool` 条件循环 |
| `src/models.py` / `apis/v1/agent.py` | 待实现 | 请求开关与 `tool_calls` 可观测响应 |
...
**API（待实现）**：计划沿用 `POST /agent/run`，增加 `agentic: bool`（默认 `false` 继续走 P2-B；`true` 走 P2-C），并在响应 `data` 增加经过脱敏的 `tool_calls` 观测信息。当前请求/响应模型中均无这些字段。
...
- [ ] mock LLM 返回固定 `tool_calls`，验证决策环路由与 `max_steps` 限制。
- [ ] 验证 P2-C 决策环在 `grounded: false` 时不编造。
...
- [x] 工具注册、行为与未知工具拒绝单测
- [ ] `llm.chat_with_tools` 与 `tool_calls` 解析
- [ ] `messages → agent_node → tool_node → agent_node` 条件循环
- [ ] `/agent/run` 的 `agentic` 开关与工具调用观测响应
- [ ] P2-C 循环上限、降级、拒答和跨课负例回归
...
**文档状态**：01 产品边界与 02 当前架构已回写 P1–P2-B 进度；P2-C 只能在决策环真正接入后标记完成。
...
P2-C   LLM 决策 + 工具调用 ◄── 工具底座完成，决策环待接入 │
...
1. P2-C 接线时保留 P2-B 作为默认和降级路径，不直接替换现有 `/agent/run`。
2. 先实测当前 LLM 是否稳定返回 `tool_calls`，再开放 `agentic` 开关。
