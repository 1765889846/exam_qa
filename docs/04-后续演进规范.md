...
- 通信学院学生查「信号与系统」，马院学生查「思政原理」，AI 课查「涌现能力」，**互不串库、互不串答**。
...
| 实体 | 说明 | 示例 |
|------|------|------|
| `college_id` | 学院标识 | `college-telecom` |
| `college` | 展示名 | 通信工程学院 |
| `course_id` | 课程标识（**检索 filter 主键**） | `course-signals-2025` |
| `course` | 展示名 | 信号与系统 |
...
新增 `services/catalog.py`（或 `storage/course_store.py`）：学院/课程 CRUD，供 apis 与校验使用。

### 4.2.6 前端（Next.js）

| 页面/组件 | 行为 |
|-----------|------|
| 全局上下文 | 当前 `college_id`、`course_id`（URL 如 `/course/[courseId]` 或 Context） |
| 选课 | `GET /colleges` → `GET /courses` → 写入 context |
| 上传页 | 带当前 `course_id` 调 `POST /documents` |
| 问答页 | 带当前 `course_id` 调 `POST /ask` |
| 顶栏 | 始终展示当前课程名，避免问错库 |
...
**负例**：用 A 课问题 + B 课 `course_id`，不得命中 A 课 chunk。
...
- [ ] SQLite 迁移脚本：`colleges`、`courses` 表 + 种子数据
- [ ] 现有文档补 `course_id`（或清空重传）
- [ ] Chunk metadata 含 `college_id`、`course_id`
- [ ] `ingest_file`、`retrieve`、`ask` 均强制 `course_id`
- [ ] `POST /documents`、`POST /ask` 缺 `course_id` → 422
- [ ] 负例测试：跨 course 检索不命中
- [ ] Next.js 选课 UI + 请求带 `course_id`
- [ ] `golden_qa.jsonl` 含 `course_id`
